    ...
    public:
        ...
        void run()
        {
            ...
            while(rclcpp::ok() && motor_running_)
            {
                ...
                double duration_time = current_time - start_time;
                for(int i = 0;i < joint_nums_;i++)
                {
                    array<double, 3> target;
                    target = fourier_serise(duration_time, A_[i], B_[i], C_[i]);
                    target_positions_[i] = target[0];
                    target_velocities_[i] = target[1];
                    target_accelerations_[i] = target[2];
                }
                // // 统计控制频率代码
                ...
                std::copy(target_velocities_.begin(), target_velocities_.begin() + 6, qd.begin());
                std::copy(target_accelerations_.begin(), target_accelerations_.begin() + 6, qdd.begin());
                // 计算前馈力矩
                ...
                //     for(int i = 0;i < joint_nums_;i++)
                //     {
                //         RCLCPP_INFO(this->get_logger(), "force[%d]: %.6f", i, force[i]);
                //     }
                // }
                // for(int i = 0;i < joint_nums_;i++)
                // {
