...
class saber_joint_controller : public rclcpp::Node
{
    public:
        ...
        saber_joint_controller(const string& name, int sock) : Node(name), sock_(sock)
        {
            ...
            // 读取最小惯性参数集
            XLDocument PA;
            PA.open("/home/xiatenghui/saber_joint_controller/src/saber_control/src/saber_min_param.xlsx");
            ...
            // 100Hz的频率发布状态
            timer_ = this->create_wall_timer(10ms,bind(&saber_joint_controller::timer_callback, this));
        ...
        void execute_trajectory_quintic_interpolation(const std::shared_ptr<rclcpp_action::ServerGoalHandle<FollowJointTrajectory>> goal_handle)
        {
            ...
            while (rclcpp::ok())
            {
                ...
                Vector<double, 6> positions;
                Vector<double, 6> t_vector;
                t_vector[0] = 1;
                ...
                Vector<double, 6> velocities;
                Vector<double, 6> t_vector_d;
                t_vector_d[0] = 0;
                t_vector_d[1] = 1;
                ...
                Vector<double, 6> accelerations;
                Vector<double, 6> t_vector_dd;
                t_vector_dd[0] = 0;
                t_vector_dd[1] = 0;
                t_vector_dd[2] = 2;
        ...
        void init()
        {
            ...
            // }
            set_id(5,motors_id[1], master_id, sock_);   // 2->5
            // set_id(2,motors_id[4], master_id, sock_);   // 5->2
        }

        void write_motors(array<double, 6> torques)
        {
            motor_controlmode(motors_id[0], torques[0], 0, 0, 60, 0.2, sock_);
            motor_controlmode(motors_id[1], torques[1], 0, 0, 60, 0.2, sock_);
            motor_controlmode(motors_id[2], torques[2], 0, 0, 60, 0.2, sock_);
            motor_controlmode(motors_id[3], torques[3], 0, 0, 60, 0.2, sock_);
            motor_controlmode(motors_id[4], torques[4], 0, 0, 60, 0.2, sock_);
            motor_controlmode(motors_id[5], torques[5], 0, 0, 60, 0.2, sock_);
        }
        ...
        void run()
        {
            ...
            while(rclcpp::ok() && motor_running_)
            {
                ...
                // 转换成电机期望的方向和角度
                array<double, 6> q, qd, qdd;
                std::copy(positions_.begin(), positions_.end(), q.begin());
                // 计算前馈力矩
                double calculate_result[216];
                saber_get_regression(calculate_result, q, qd, qdd, 9.81);
                Map<Matrix<double, 6, 36, RowMajor>> Y_r(calculate_result);
                Map<Matrix<double, 6, 1>> vel(qd.data());
                Matrix<double, 6, 6> Y_v = vel.asDiagonal();
                MatrixXd Y_c = MatrixXd::Identity(6, 6);
                Matrix<double, 6, 48> Regression_Matrix;
                Regression_Matrix << Y_r, Y_c, Y_v;
                VectorXd force = Regression_Matrix * PA_;
                for(int i = 0;i < joint_nums_;i++)
                {
                    q[i] = q[i] * DH2motor_directions_[i];
                    feedforward_torques[i] = force[i] * DH2motor_directions_[i];
                }
                write_motors(feedforward_torques);
                loop_rate.sleep();
